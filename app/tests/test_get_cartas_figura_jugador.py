...
def test_get_cartas_figura_jugador_con_creador_y_otro_jugador(test_db):
    with mock.patch('crud.partidas.random_figura', mock.Mock(return_value='f1')):
        ...
        response = client.get(f"/partidas/juego/{{partida_id: int}}/jugadores/{{jugador_id: int}}/cartas_figura?partida_id={id_partida}&jugador_id={creador_id}")
        assert response.status_code == 200, f"Fallo: Se esperaba el estado 200, pero se obtuvo {response.status_code}"
        ...
        response = client.get(f"/partidas/juego/{{partida_id: int}}/jugadores/{{jugador_id: int}}/cartas_figura?partida_id={id_partida}&jugador_id={otro_jugador_id}")
        assert response.status_code == 200, f"Fallo: Se esperaba el estado 200, pero se obtuvo {response.status_code}"
