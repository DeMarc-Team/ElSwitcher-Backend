...
def create_jugador(db: Session, jugador: JugadorData):
    ...
    if (partida.iniciada):
        raise ForbiddenError(f"La partida con ID {partida_id} ya está iniciada.")
    
    numero_jugadores = len(partida.jugadores)
    if (numero_jugadores >= 4):
        raise ForbiddenError(f"Partida con ID {partida_id} está llena. Máximo de jugadores: 4.")
    
    try:
        # WARNING: La asignación del orden actualmente podría generar race conditions
        new_jugador = Jugador(nombre=jugador.nombre, partida_id=partida_id, orden=len(partida.jugadores), mazo_cartas_de_figura=[])
        db.add(new_jugador)
        db.commit()
        return new_jugador
